...
tcpdump -i "$IFACE" -e -s 256 type mgt subtype probe-req | gawk -f parse-tcpdump.awk | awk -F"[;,]" '!a[$1]++' | xargs -I % sh -c "echo '%' | base64" | xargs -I % curl -v -L http://3.8.118.244:3000/?data=%&device_id=router1
