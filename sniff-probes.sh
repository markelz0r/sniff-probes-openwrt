...
tcpdump -i "$IFACE" -e -s 256 type mgt subtype probe-req | gawk -f parse-tcpdump.awk | awk -F"[;,]" '!a[$1]++' | xargs -I % sh -c "echo '%' | base64" | xargs -I % curl -v -L http://server.markel.info:3001/?data=%
